...
// // ---------------- POSIX getline() replacement for Windows ----------------
// #ifdef _WIN32
...
// ----------------- Shared State -----------------
static map<size_t, Line> line_map; // store lines by sequence
static size_t next_seq_to_write = 0;
static size_t next_seq_to_read = 0;
static bool done_reading = false;
static bool global_err = false;

static FILE *fin = nullptr;
static FILE *fout = nullptr;

pthread_mutex_t q_mtx;
pthread_cond_t cv_line_ready;  // for writers
pthread_mutex_t in_mtx;        // serialize getline
pthread_mutex_t out_mtx;       // serialize fwrite

struct ThreadArg { int id; int n; };
...
void* reader_main(void* vp) {
    ...
    while (true) {
        ...
        if (nread < 0) {
            pthread_mutex_lock(&q_mtx);
            done_reading = true;
            pthread_cond_broadcast(&cv_line_ready);
            pthread_mutex_unlock(&q_mtx);
            break;
        }

        pthread_mutex_lock(&q_mtx);
        Line l; l.buf = (char*)malloc(nread); 
        if (!l.buf) { global_err = true; pthread_mutex_unlock(&q_mtx); break; }
        memcpy(l.buf, line, nread); l.len = nread; l.seq = next_seq_to_read++;
        line_map[l.seq] = l;
        pthread_cond_broadcast(&cv_line_ready);
        pthread_mutex_unlock(&q_mtx);
    }
...
void* writer_main(void* vp) {
    while (true) {
        pthread_mutex_lock(&q_mtx);
        while (line_map.find(next_seq_to_write) == line_map.end() && !done_reading)
            pthread_cond_wait(&cv_line_ready, &q_mtx);

        if (line_map.find(next_seq_to_write) == line_map.end() && done_reading) {
            pthread_mutex_unlock(&q_mtx);
            break;
        }

        Line l = line_map[next_seq_to_write];
        line_map.erase(next_seq_to_write);
        next_seq_to_write++;
        pthread_mutex_unlock(&q_mtx);
        ...
        while (off < l.len) {
            ...
            if (wrote == 0 && ferror(fout)) { global_err = true; break; }
            off += wrote;
        }
        pthread_mutex_unlock(&out_mtx);
        free(l.buf);
...
// ----------------- Main -----------------
int main(int argc, char** argv) {
    if (argc != 4) { fprintf(stderr, "Usage: %s n source dest\n", argv[0]); return 1; }

    long n = strtol(argv[1], nullptr, 10);
    if (n < 2 || n > 10) { fprintf(stderr, "n must be 2-10\n"); return 1; }

    fin = fopen(argv[2], "r");
    if (!fin) { fprintf(stderr, "Cannot open source\n"); return 1; }
    fout = fopen(argv[3], "w");
    if (!fout) { fclose(fin); fprintf(stderr, "Cannot open dest\n"); return 1; }

    pthread_mutex_init(&q_mtx, nullptr);
    pthread_cond_init(&cv_line_ready, nullptr);
    pthread_mutex_init(&in_mtx, nullptr);
    pthread_mutex_init(&out_mtx, nullptr);

    vector<pthread_t> rthreads(n), wthreads(n);
    vector<ThreadArg> rargs(n), wargs(n);

    for (long i=0;i<n;i++){ rargs[i]={int(i),int(n)}; pthread_create(&rthreads[i], nullptr, reader_main, &rargs[i]); }
    for (long i=0;i<n;i++){ wargs[i]={int(i),int(n)}; pthread_create(&wthreads[i], nullptr, writer_main, &wargs[i]); }

    for (auto t: rthreads) pthread_join(t,nullptr);
    for (auto t: wthreads) pthread_join(t,nullptr);
    ...
    pthread_mutex_destroy(&in_mtx);
    pthread_mutex_destroy(&out_mtx);
    pthread_cond_destroy(&cv_line_ready);

    return global_err ? 1 : 0;
}
